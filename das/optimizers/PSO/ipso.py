...
class IPSO(SubOptimizer):
    ...
    def iterate(self, v, x, y, p_x, p_y):
        ...
        if self.n_individuals < self.max_n_individuals and not self._check_terminations():
    ...
    def set_data(self, x=None, y=None, best_x=None, best_y=None, **kwargs):
        ...
        if x is not None and y is not None and len(x) >= 1:
            ...
            v1  = v_kwarg[best_idx : best_idx + 1] if v_kwarg is not None else np.zeros((1, self.ndim_problem))
            p_x_kwarg = kwargs.get("p_x")
            p_x1 = p_x_kwarg[best_idx : best_idx + 1] if p_x_kwarg is not None else np.copy(x1)
            p_y_kwarg = kwargs.get("p_y")
            p_y1 = p_y_kwarg[best_idx : best_idx + 1] if p_y_kwarg is not None else np.copy(y1)

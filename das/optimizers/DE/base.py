...
class DE(SubOptimizer):
    ...
    def _crossover(self, target, donor):
        ...
        return np.clip(
            np.where(mask, donor, target), self.lower_boundary, self.upper_boundary
        )
    ...
    def iterate(self, x, y):
        for i in range(self.n_individuals):
            if self._check_terminations():
                return x, y
            trial_y = self._evaluate_fitness(
                self._crossover(x[i], self._mutate(x, y, i))
            )
            if trial_y <= y[i]:
...
class _SHADEBase(DE):
    ...
    def _choose_F_Cr(self, NP: int):
        ...
        Cr = np.clip(
            self.rng_optimization.normal(loc=self.MCr[idx], scale=0.1), 0.0, 1.0
        )
        locs = self.MF[idx]
    ...
    def _update_memory(self, SF, SCr, df):
        if len(SF) > 0:
            w = df / np.sum(df)
            self.MF[self.k_idx] = (
                np.sum(w * SF**2) / np.sum(w * SF) if np.sum(w * SF) > 1e-6 else 0.5
            )
            self.MCr[self.k_idx] = (
                np.sum(w * SCr**2) / np.sum(w * SCr) if np.sum(w * SCr) > 1e-6 else 0.5
            )
        else:
    ...
    def _nlpsr(self, x, y, A_rate: float = 2.1):
        ...
        new_NP = max(
            self.Nmin,
            int(np.round(self.Nmax + (self.Nmin - self.Nmax) * ratio ** (1.0 - ratio))),
        )
        if new_NP < x.shape[0]:
    ...
    def _unique_indices(
        self, exclude: list[int], pool_size: int, n: int, max_tries: int = 25
    ) -> np.ndarray:

    ...
    def iterate(self, x, y):
        ...
        for i in range(NP):
            ...
            if use_arc[i] and len(self.archive) > 0:
                x2[i] = self.archive[
                    self.rng_optimization.integers(0, min(len(self.archive), self.NA))
                ]
            else:
                r2_i = int(self.rng_optimization.choice(NP, p=pr))
                for _ in range(25):
                    if r2_i not in (i, pb_i, r1_i):
                        break
                    r2_i = int(self.rng_optimization.choice(NP, p=pr))
                x2[i] = x[r2_i]
        ...
        if self.rng_optimization.random() < 0.5:
            ...
            L = self.rng_optimization.integers(dim, size=NP).repeat(dim).reshape(
                NP, dim
            ) <= np.arange(dim)
            mask = np.where(
                self.rng_optimization.random((NP, dim)) > Cr[:, None], L, False
            )
            u = np.where(mask, v, u)
        ...
        if better.any():
            ...
            if na > 0 and fa > 0:
                self.pa = np.clip(
                    fa / (na + 1e-15) / (fa / (na + 1e-15) + fp / (NP - na + 1e-15)),
                    0.1,
                    0.9,
                )

...
class JDE21(DE):
    ...
    def __init__(self, problem: dict, options: dict):
        ...
        self.bNP = 160; self.sNP = 10
        options["n_individuals"] = self.bNP + self.sNP
        super().__init__(problem, options)

        self.tau1 = 0.1;   self.tau2   = 0.1
        self.Fl_b = 0.1;   self.Fu     = 1.1
        self.CRl_b = 0.0;  self.CRu_b  = 1.1
        self.Fl_s  = 0.17; self.CRl_s  = 0.1; self.CRu_s = 0.8
        self.Finit = 0.5;  self.CRinit = 0.9
    ...
    def _adapt(self, i, is_big):
        ...
        F  = Fl + self.rng_optimization.random() * (self.Fu - Fl) \
            if self.rng_optimization.random() < self.tau1 else self._F[i]
        CRl = self.CRl_b if is_big else self.CRl_s
        CRu = self.CRu_b if is_big else self.CRu_s
        CR  = CRl + self.rng_optimization.random() * (CRu - CRl) \
            if self.rng_optimization.random() < self.tau2 else self._CR[i]
        return F, CR
    ...
    def iterate(self, x, y):
        ...
        if self._age >= self.age_limit:
            self._age = 0
            for slot in (int(np.argmax(y[: self.bNP])), self.bNP + int(np.argmin(y[self.bNP :]))):
                x[slot] = self.rng_initialization.uniform(
                    self.initial_lower_boundary, self.initial_upper_boundary, self.ndim_problem
                )
    ...
    def initialize(self, x=None, y=None):
        F  = self._warm_start.get("F");  CR = self._warm_start.get("CR")
        if F  is not None: self._F  = np.copy(F)
        if CR is not None: self._CR = np.copy(CR)
    ...
    def set_data(self, x=None, y=None, best_x=None, best_y=None, **kwargs):
        if x is None or y is None or len(x) < self.n_individuals:
            ...
            F_kw  = kwargs.get("F");  CR_kw = kwargs.get("CR")
            if F_kw  is not None: self._F  = np.copy(F_kw)
            if CR_kw is not None: self._CR = np.copy(CR_kw)
            self._warm_start = {
                "x": x[: self.n_individuals], "y": y[: self.n_individuals],
                "F": self._F, "CR": self._CR,
            }
